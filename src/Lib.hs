...
newtype Outcome = Outcome String deriving (Show, Eq)


data Game = AtomGame {players :: Int
                 , states :: [State]
                 , outcomes :: [Outcome]
...
instance Show Game where
  show g@(AtomGame _ _ _ _ _ _) = (show $ players g) ++ ", " ++ (show $ states g) ++ ", " ++ (show $ outcomes g)
...
fptpVote k a = AtomGame { players = k
...
lptpElimination k a = AtomGame {players = k
                           , states = [State $ show i | i <- [0..a]]
                           , outcomes = [Outcome $ show i | i <- [1..a]]
                           , label = \(State i) -> Outcome i
                           , playerMoves = \(Player p) (State s) -> if s=="0"
...
alternativeVote k a = AtomGame { players = k
                     , states = [State $  show i | i <- [0..a]]
                     , outcomes = [Outcome $ show i | i <- [1..a]]
                     , label = \(State i) -> Outcome i
                     , playerMoves = \(Player p) (State s) -> if s=="0"
